...
interface PerformanceAnalyticsItem {
  id: string;
  title?: string;
  status?: string;
  createdAt: string;
...
export default function PerformanceAnalyticsPage() {
  const [items, setItems] = useState<PerformanceAnalyticsItem[]>([]);
  ...
  const handleDelete = async (id: string) => {
    await fetch(`/api/performance-analytics/${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <p className="text-gray-600 mb-6">Comprehensive PIP performance insights</p>
