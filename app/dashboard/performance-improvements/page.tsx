...
interface PerformanceImprovementsItem {
  id: string;
  title?: string;
  status?: string;
  createdAt: string;
...
export default function PerformanceImprovementsPage() {
  const [items, setItems] = useState<PerformanceImprovementsItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [showCreate, setShowCreate] = useState(false);
  const [newTitle, setNewTitle] = useState('');

  useEffect(() => {
    fetch('/api/performance-improvements')
      .then(r => r.json())
  ...
  const handleCreate = async () => {
    ...
    const res = await fetch('/api/performance-improvements', {
      method: 'POST',
  ...
  const handleDelete = async (id: string) => {
    await fetch(`/api/performance-improvements/${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Performance Improvement Dashboard</h1>
        <button
      ...
      <p className="text-gray-600 mb-6">Manage and track active PIPs</p>
